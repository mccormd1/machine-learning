...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing == True:
            ...
            self.trial+=1
            #self.epsilon-=.003  #linear decay
#             self.epsilon-=.002 #linear decay
#              self.epsilon=self.alpha**(self.trial/26.0) #exponential decay
            fulltraintrial=240 #step function decay
            totaltrial=260
            if self.trial < fulltraintrial:
                #self.epsilon=self.epsilon-(.5/fulltraintrial) #50% random by 80 trials deep 
                self.epsilon=1
    ...
    def createQ(self, state):
        ...
        for actions in self.valid_actions:
            self.Q[state][actions]=self.Q.get(state,{}).get(actions,0)
            #self.Q[state][actions]=self.Q.get(state,{}).get(actions,random.uniform(-.5,.5))
        #print(max(self.Q[state], key=self.Q.get))
        return
    ...
    def choose_action(self, state):
        ...
        if self.learning == False:
            ...
            epdecision=random.random()
            if epdecision <= self.epsilon:
                action = random.choice(self.valid_actions)
            else:
